...
- `background.test.js`: Node regression coverage for the WebSocket manual state machine, session-token loading, and ChatGPT/Claude target toggling.
...
- `socket`: Active WebSocket connection to Electron at `ws://localhost:8080`, authenticated first with the per-session token generated by Electron.
